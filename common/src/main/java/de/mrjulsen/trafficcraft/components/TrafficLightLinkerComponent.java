...
public record TrafficLightLinkerComponent(Optional<Location> location, LinkerMode mode, Optional<String> targetBlockName) {

    public static final Codec<TrafficLightLinkerComponent> CODEC = RecordCodecBuilder.create(builder -> {
        return builder.group(
            CompoundTag.CODEC.optionalFieldOf("location").xmap((b) -> b.map(Location::fromNbt), (b) -> b.map(Location::toNbt)).forGetter(TrafficLightLinkerComponent::location),
    ...
    public static final StreamCodec<ByteBuf, TrafficLightLinkerComponent> STREAM_CODEC = StreamCodec.composite(
        ByteBufCodecs.COMPOUND_TAG.apply(ByteBufCodecs::optional).map((b) -> b.map(Location::fromNbt), (b) -> b.map(Location::toNbt)), TrafficLightLinkerComponent::location,
