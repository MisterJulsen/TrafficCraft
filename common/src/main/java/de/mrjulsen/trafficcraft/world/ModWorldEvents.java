...
public class ModWorldEvents {    
    ...
    public static final Holder<ConfiguredFeature<DiskConfiguration, ?>> SALT = FeatureUtils.register(TrafficCraft.MOD_ID + ":salt",
        Feature.DISK, new DiskConfiguration(ModBlocks.SALT.get().defaultBlockState(), UniformInt.of(5, 8), 1, List.of(Blocks.GRAVEL.defaultBlockState(), Blocks.SAND.defaultBlockState())));

    public static void init() {

        Holder<PlacedFeature> bitumenOrePlacedFeature = PlacementUtils.register(
        ...
        Holder<PlacedFeature> saltDiskPlacedFeature = PlacementUtils.register(
        ...
        BiomeModifications.addProperties((ctx, mutable) -> {
            mutable.getGenerationProperties().addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, bitumenOrePlacedFeature);
            mutable.getGenerationProperties().addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, rockSaltOrePlacedFeature);
            if (ctx.getProperties().getCategory() == BiomeCategory.OCEAN) {
                mutable.getGenerationProperties().addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, saltDiskPlacedFeature);
            }
