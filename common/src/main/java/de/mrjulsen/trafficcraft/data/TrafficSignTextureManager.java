...
public class TrafficSignTextureManager {
    ...
    public static TrafficSignTextureData load(String id) {
        ...
        Path path = GameInstance.getServer().getWorldPath(getResource(id.toString()));
        if (path.toFile().exists()) {
            try {
                return TrafficSignTextureData.deserializeNbt(NbtIo.readCompressed(path, NbtAccounter.unlimitedHeap()));
            } catch (IOException e) {
