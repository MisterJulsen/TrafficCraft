...
public class MainScreen extends DLGuiComponent {
    ...
    public MainScreen(TrafficSignWorkbenchWindow win) {
        ...
        btnEdit.addEventListener(DLGuiStandardEvents.ClickEvent.class, (s, e) -> {
            ...
            win.clearComponents();
            win.addComponent(new EditorScreen(
                    win,
                    getPrevievTexture().getRawData().getShape(),
                    getPrevievTexture().getTexture(),
                    preview.getName(),
                    (win.getMenu().patternSlot.getItem().getItem() instanceof PatternCatalogueItem item) ? item.getSelectedIndex(win.getMenu().patternSlot.getItem()) : -1
            ));
            return false;
        });
        ...
        btnDelete.addEventListener(DLGuiStandardEvents.ClickEvent.class, (s, e) -> {
            ...
            Minecraft.getInstance().setScreen(new ConfirmScreen((b) -> {
                if (b && win.getMenu().patternSlot.getItem().getItem() instanceof PatternCatalogueItem item) {
                    int idx = item.getSelectedIndex(win.getMenu().patternSlot.getItem());
    ...
    private void switchPreview(int index) {
        if (!(win.getMenu().patternSlot.getItem().getItem() instanceof PatternCatalogueItem item)) {
            return;
        }
        int dx = MathUtils.clamp(item.getSelectedIndex(win.getMenu().patternSlot.getItem()) + index, 0, item.getStoredPatternCount(win.getMenu().patternSlot.getItem()) - 1);
        item.setSelectedIndex(win.getMenu().patternSlot.getItem(), dx);
    ...
    private void initPreview() {
        ...
        this.preview = item.getSelectedPattern(win.getMenu().patternSlot.getItem());
        getPrevievTexture();
    }
    ...
    public void renderMainLayer(DLGuiGraphics graphics, double mouseX, double mouseY, Rectangle renderBounds) {
        ...
        label = (win.getMenu().patternSlot.getItem().getItem() instanceof PatternCatalogueItem item) ? String.format("%s / %s", item.getSelectedIndex(win.getMenu().patternSlot.getItem()) + 1, item.getStoredPatternCount(win.getMenu().patternSlot.getItem())) : "";
