...
public class TrafficSignPatternSelectionScreen extends DLWindow {
    ...
    private void loadTextures() {
        ...
        if (selectedShape != null && bookmarks.contains(selectedShape) && item instanceof CreativePatternCatalogueItem creativeItem) {
            // builtin textures
            ...
            for (int i = 0; i < count; i++) {
                ...
                textureBtn.tooltip.set(new DLTooltip(List.of(TextUtils.text(item.getPatternAt(stack, j).getName())), 200));
            }
        }
    }
    ...
    public void close() {
        if (selectedShape == null) {
            ModNetworkManager.UPDATE_PATTERN_CATALOG_INDEX.send(NetworkDirection.toServer(), new PatternCatalogueIndexPacket(item.getSelectedIndex(stack)));
