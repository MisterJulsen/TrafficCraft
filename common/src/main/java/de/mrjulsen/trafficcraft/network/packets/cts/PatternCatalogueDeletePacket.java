...
public class PatternCatalogueDeletePacket extends BaseNetworkPacket<PatternCatalogueDeletePacket> {
    private int index;
    ...
    @Override
    public void encode(PatternCatalogueDeletePacket packet, RegistryFriendlyByteBuf buffer) {
        buffer.writeInt(packet.index);
    }

    @Override
    public PatternCatalogueDeletePacket decode(RegistryFriendlyByteBuf buffer) {
    ...
    public void handle(PatternCatalogueDeletePacket packet, Supplier<PacketContext> contextSupplier) {
        contextSupplier.get().queue(() -> {
            ...
            if (sender.containerMenu instanceof TrafficSignWorkbenchMenu menu) {
                ...
                item.removePatternAt(stack, packet.index);
