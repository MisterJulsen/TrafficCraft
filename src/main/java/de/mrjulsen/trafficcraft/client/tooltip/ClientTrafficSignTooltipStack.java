    ...
    public int getWidth(Font pFont) {
        ...
        if (true) { //(Screen.hasShiftDown()) {
            ...
            for (int i = 0; i < this.data.size(); i++) {
                ...
                int textWidth = (int)(pFont.width(TextUtils.text(label).withStyle(selectedIndex == i ? ChatFormatting.BOLD : ChatFormatting.RESET)) * FONT_SCALE);
    ...
    public void renderImage(Font pFont, int pMouseX, int pMouseY, PoseStack pPoseStack, ItemRenderer pItemRenderer, int pBlitOffset) {
        ...
        for (int i = 0; i < this.data.size(); i++) {
            ...
            if (true) { //(Screen.hasShiftDown()) {
                ...
                MutableComponent label = TextUtils.text(txt).withStyle(selectedIndex == i ? ChatFormatting.BOLD : ChatFormatting.RESET).withStyle(selectedIndex == i ? ChatFormatting.WHITE : ChatFormatting.GRAY);
