...
public class LerpedFloat {
	...
	public static LerpedFloat linear() {
		return new LerpedFloat((p, c, t) -> (float) Math.lerp(p, c, t));
	}
	...
	public interface Chaser {
		...
		Chaser LINEAR = (c, s, t) -> (float) (c + Math.clamp(t - c, -s, s));

		static Chaser exp(double maxEffectiveSpeed) {
			return (c, s, t) -> (float) (c + Math.clamp((t - c) * s, -maxEffectiveSpeed, maxEffectiveSpeed));
		}
