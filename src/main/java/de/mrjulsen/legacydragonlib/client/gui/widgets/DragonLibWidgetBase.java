...
@OnlyIn(Dist.CLIENT)
public abstract class DragonLibWidgetBase extends GuiComponent implements Widget, GuiEventListener, NarratableEntry {
    ...
    public DragonLibWidgetBase(int pX, int pY, int pWidth, int pHeight, Component pMessage) {
        this.x = pX;
        this.y = pY;
        this.width = pWidth;
        this.height = pHeight;
    ...
    protected void defaultButtonNarrationText(NarrationElementOutput p_168803_) {
        ...
        if (this.active) {
            if (this.isFocused()) {
                p_168803_.add(NarratedElementType.USAGE, Utils.translate("narration.button.usage.focused"));
            } else {
                p_168803_.add(NarratedElementType.USAGE, Utils.translate("narration.button.usage.hovered"));
            }
        }
